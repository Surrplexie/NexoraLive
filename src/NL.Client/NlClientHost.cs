...
public sealed class NlClientHost
{
    ...
    public async Task<IReadOnlyList<NlClientStreamerInfo>> ListStreamersAsync(
        IEnumerable<string>? additionalStreamerIds = null,
        CancellationToken cancellationToken = default)
    {
        ...
        if (additionalStreamerIds is not null)
        {
            foreach (var extra in additionalStreamerIds)
            {
                ...
                if (list.Any(s => string.Equals(s.StreamerId, extra, StringComparison.OrdinalIgnoreCase)))
                {
                    continue;
                }

                list.Add(await BuildStreamerInfoAsync(extra.Trim(), profile, cancellationToken));
            }
        }
...
internal sealed class InProcessNlClientSessionApi : INlClientSessionApi
{
    ...
    public Task<IReadOnlyList<NlClientStreamerInfo>> ListStreamersAsync(CancellationToken cancellationToken = default) =>
        _host.ListStreamersAsync(null, cancellationToken);
