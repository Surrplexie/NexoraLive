...
IAsyncDisposable? connectListener = null;
if (game == ForkGameKind.RimWorld)
{
    var port = int.TryParse(connectPortRaw, out var parsed) && parsed > 0 ? parsed : 25555;
    var rim = new RimWorldConnectListener(port);
    rim.Start();
    connectListener = rim;
    log($"[fork] rimworld connect listener rimworld://0.0.0.0:{rim.BoundPort}");
}
else if (game == ForkGameKind.Kenshi)
{
    var port = int.TryParse(connectPortRaw, out var parsed) && parsed > 0 ? parsed : 23386;
    var ken = new KenshiConnectListener(port);
    ken.Start();
    connectListener = ken;
    log($"[fork] kenshi connect listener kenshi://0.0.0.0:{ken.BoundPort}");
}
...
{
    if (connectListener is not null)
    {
        await connectListener.DisposeAsync();
    }
}
...
{
    Console.WriteLine("""
NL.Fork.Runtime — Phase P/T1 game fork runtimes (hello / minecraft / beamng / rimworld / kenshi)

  --game hello|minecraft|beamng|rimworld|kenshi  Game profile (default hello)
