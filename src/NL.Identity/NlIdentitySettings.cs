...
public sealed class NlIdentitySettings
{
    ...
    public string EmailMode { get; init; } = "mock";
    ...
    public static NlIdentitySettings LoadFromEnvironment()
    {
        ...
        var verificationEnabled = !string.Equals(
            Environment.GetEnvironmentVariable("NL_VERIFICATION_ENABLED"),
            "0",
            StringComparison.OrdinalIgnoreCase)
            && !string.Equals(
                Environment.GetEnvironmentVariable("NL_VERIFICATION_ENABLED"),
        ...
        var emailMode = Environment.GetEnvironmentVariable("NL_EMAIL_MODE")?.Trim().ToLowerInvariant();
        if (string.IsNullOrWhiteSpace(emailMode))
        {
            emailMode = !string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("NL_EMAIL_SMTP_HOST"))
                ? "smtp"
                : "mock";
        }
    ...
    public object ToPublicInfo() => new
    {
        ...
        verificationEnabled = VerificationEnabled,
        emailMode = EmailMode,
        smtpConfigured = !string.IsNullOrWhiteSpace(Environment.GetEnvironmentVariable("NL_EMAIL_SMTP_HOST")),
        ...
        oauth = new
        {
            ...
            verifyUi = "/account-verify.html",
            loginUi = "/login.html",
        },
        unifiedLoginEnabled = true,
...
public sealed class NlIdentityHost
{
    ...
    public NlIdentityHost(NlIdentitySettings settings)
    {
        ...
        EmailChallenges = new JsonEmailVerificationChallengeStore();
        var emailSender = settings.EmailMode == "smtp"
            ? new SmtpEmailSender()
            : (IEmailSender)new MockEmailSender();
        Verification = new AccountVerificationService(Identity, Store, Audit, EmailChallenges, emailSender);
        LoginSessions = new JsonLoginSessionStore();
        UnifiedAccounts = new NlUnifiedAccountService(Identity, Store, Audit, Verification, LoginSessions);
    }

    public void AttachUnifiedProvisioner(IUnifiedAccountProvisioner provisioner)
    {
        UnifiedAccounts = new NlUnifiedAccountService(
            Identity,
            Store,
            Audit,
            Verification,
    ...
    public JsonEmailVerificationChallengeStore EmailChallenges { get; }

    public AccountVerificationService Verification { get; }

    public JsonLoginSessionStore LoginSessions { get; }

    public NlUnifiedAccountService UnifiedAccounts { get; private set; }
