...
public sealed class NlJoinAdmissionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            var account = identity.Identity.GetAccount(request.NlAccountId.Trim());
            if (account is null)
            {
                return NlJoinAdmissionResult.FromJoinResult(
                    JoinResult.Deny($"NL account '{request.NlAccountId}' not found."),
                    playerId,
                    spProfile.GetRelationship(_streamerId).Standing);
            }

            identity.Verification.ApplyVerificationToProfile(account, spProfile);

            var requiresTwoFactorStepUp =
                (_requirements.RequiredVerification & SpVerification.TwoFactor) != 0
                && account.TwoFactorEnabledAtUtc is not null;

            if (requiresTwoFactorStepUp)
            {
                if (string.IsNullOrWhiteSpace(request.TwoFactorCode)
                    || !identity.Verification.VerifyTwoFactorCode(account, request.TwoFactorCode))
                {
                    return NlJoinAdmissionResult.FromJoinResult(
                        JoinResult.Deny("Valid two-factor authentication code required."),
                        playerId,
                        spProfile.GetRelationship(_streamerId).Standing);
                }
            }
