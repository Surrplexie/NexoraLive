...
public sealed class FleetRelayService
{
    ...
    {
        if (string.IsNullOrWhiteSpace(rawConnectEndpoint))
        {
            return new FleetRelayConnectInfo(rawConnectEndpoint, rawConnectEndpoint, regionId);
        }

        // T2-lite: native game TCP (RimWorld Together, Minecraft) must stay
        // scheme://public-host:port — never smash to wss://relay.
        if (PublicForkConnect.IsNativeGameConnect(rawConnectEndpoint))
        {
            var published = PublicForkConnect.RewriteForPublic(rawConnectEndpoint);
            return new FleetRelayConnectInfo(published, rawConnectEndpoint, regionId);
        }

        if (!_config.MaskRawHostIps)
        {
            return new FleetRelayConnectInfo(rawConnectEndpoint, rawConnectEndpoint, regionId);
        }
