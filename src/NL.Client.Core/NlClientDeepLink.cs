...
public static class NlClientLaunchBuilder
{
    public static NlClientLaunchParams Build(NlClientManifest manifest)
    {
        ...
        var clipboard = NativeClipboard(fork);
    ...
    public static string? NativeClipboard(string forkEndpoint)
    {
        if (string.IsNullOrWhiteSpace(forkEndpoint))
        {
            return null;
        }

        var sep = forkEndpoint.IndexOf("://", StringComparison.Ordinal);
        if (sep <= 0)
        {
            return null;
        }

        var scheme = forkEndpoint[..sep];
        if (scheme.Equals("rimworld", StringComparison.OrdinalIgnoreCase)
            || scheme.Equals("kenshi", StringComparison.OrdinalIgnoreCase)
            || scheme.Equals("minecraft", StringComparison.OrdinalIgnoreCase)
            || scheme.Equals("beamng-sidecar", StringComparison.OrdinalIgnoreCase))
        {
            return forkEndpoint;
        }
