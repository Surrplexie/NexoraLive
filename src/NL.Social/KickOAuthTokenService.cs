...
/// <summary>Refreshes and caches per-player Kick user access tokens.</summary>
public sealed class KickOAuthTokenService
{
    ...
    public async Task<string?> GetValidAccessTokenAsync(string playerId, CancellationToken cancellationToken = default)
    {
        ...
        var body = await response.Content.ReadFromJsonAsync<KickTokenResponse>(cancellationToken);
