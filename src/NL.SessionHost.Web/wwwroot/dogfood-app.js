(function () {
  ...
  document.getElementById('btn-setup').onclick = async function () {
    try {
      ...
      var profile = (data.status && data.status.profile) ? data.status.profile : data.profile;
      document.getElementById('setup-result').textContent = JSON.stringify(profile, null, 2);
