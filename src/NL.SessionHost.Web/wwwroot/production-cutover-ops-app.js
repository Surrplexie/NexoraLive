...
async function loadStatus() {
  document.getElementById('status-block').textContent = JSON.stringify(await api('/api/v1/production-cutover/status'), null, 2);
}

async function loadSettings() {
  document.getElementById('settings-block').textContent = JSON.stringify(await api('/api/v1/production-cutover/settings'), null, 2);
}

async function loadValidation() {
  const body = {
    publicHttpsVerified: document.getElementById('https-verified').checked,
  ...
  const report = await api('/api/v1/production-cutover/validation/run', { method: 'POST', body: JSON.stringify(body) });
  const summary = document.getElementById('validation-summary');
  summary.textContent = report.productionCutoverPassed ? 'PRODUCTION CUTOVER VALIDATION PASSED' : 'PRODUCTION CUTOVER VALIDATION FAILED';
  summary.className = report.productionCutoverPassed ? 'ok' : 'warn';
