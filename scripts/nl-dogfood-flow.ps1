...
function Get-DockerImageBuild([string]$Id) {
    switch ($Id.Trim().ToLowerInvariant()) {
        ...
        "rimworld" { return @{ Dockerfile = "docker/fork-rimworld/Dockerfile"; Tag = "nl-fork-rimworld:latest" } }
        "kenshi" { return @{ Dockerfile = "docker/fork-kenshi/Dockerfile"; Tag = "nl-fork-kenshi:latest" } }
        "example-game" { return @{ Dockerfile = "docker/fork-example-game/Dockerfile"; Tag = "nl-fork-example-game:latest" } }
        default { return @{ Dockerfile = "docker/fork-hello/Dockerfile"; Tag = "nl-fork-hello:latest" } }
    }
}

$shouldVerifyRules = $VerifyRuleEvents.IsPresent -or ($GameId -eq "minecraft") -or ($GameId -eq "rimworld") -or ($GameId -eq "kenshi")
...
Step "Dogfood setup (profile + mock ownership)" {
    $setupRaw = Invoke-Nl POST "/api/v1/dogfood/setup" @{ gameId = $GameId }
...
Step "Start session + fork provision" {
    ...
    if ($ExpectProvisioner -eq "docker") {
        ...
        $validConnect = $connect -match '^(docker|minecraft|beamng-sidecar|rimworld|kenshi)://'
        if (-not $validConnect) {
            throw ("Expected docker://, minecraft://, beamng-sidecar://, rimworld://, or kenshi:// connect URL, got {0}" -f $connect)
        }
    }
}
