...
bash "$repo/scripts/nl-vps-preflight.sh"
...
if grep -qE '^STEAM_WEB_API_KEY=\s*$' "$fleet_env"; then
  echo "WARN: STEAM_WEB_API_KEY is empty — live ownership joins will fail. Set it before inviting players." >&2
fi
...
echo "Building fork images (hello-fork, minecraft, rimworld)..."
bash "$repo/scripts/build-fork-images.sh" hello-fork,minecraft,rimworld
