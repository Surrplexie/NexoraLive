...
$runMinecraft = $AllGames -or $PublicLine
$runBeamng = $AllGames
$runRimworld = $AllGames -or $PublicLine
$runKenshi = $AllGames
...
if ($runRimworld) {
    Write-Host "rimworld Docker dogfood..." -ForegroundColor Yellow
    & (Join-Path $Root "scripts/nl-dogfood-flow.ps1") `
        -BaseUrl $BaseUrl `
        -StreamerId $StreamerId `
        -PlayerId ($PlayerId + "-rw") `
        -Steam64 $Steam64 `
        -GameId rimworld `
        -ExpectProvisioner docker `
        -SkipImageBuild `
        ...
        -NlAccountId $accountId `
        -VerifyRuleEvents
    if ($LASTEXITCODE -ne 0) { throw "rimworld dogfood failed" }
    $rimworldJoinVerified = $true
    Write-Host "OK: rimworld join + teardown" -ForegroundColor Green
}

if ($runKenshi) {
    Write-Host "kenshi Docker dogfood..." -ForegroundColor Yellow
    & (Join-Path $Root "scripts/nl-dogfood-flow.ps1") `
        -BaseUrl $BaseUrl `
        -StreamerId $StreamerId `
        -PlayerId ($PlayerId + "-ken") `
        -Steam64 $Steam64 `
        -GameId kenshi `
        -ExpectProvisioner docker `
        -SkipImageBuild `
        ...
        -NlAccountId $accountId `
        -VerifyRuleEvents
    if ($LASTEXITCODE -ne 0) { throw "kenshi dogfood failed" }
    $kenshiJoinVerified = $true
    Write-Host "OK: kenshi join + teardown" -ForegroundColor Green
}
...
$body = @{
    ...
    rimworldJoinVerified = $rimworldJoinVerified
    kenshiJoinVerified = $kenshiJoinVerified
    forkTeardownVerified = $forkTeardownVerified
    streamerId = $StreamerId
    verifiedGames = @("hello-fork")
}
if ($AllGames) {
    $body.verifiedGames = @("hello-fork", "minecraft", "beamng", "rimworld", "kenshi")
} elseif ($PublicLine) {
    $body.verifiedGames = @("hello-fork", "minecraft", "rimworld")
}
