...
public class FleetOpsTests
{
    ...
    [Fact]
    public void RelayMasking_KeepsRimWorldNativeScheme()
    {
        var previous = Environment.GetEnvironmentVariable("NL_FORK_PUBLIC_CONNECT_HOST");
        try
        {
            Environment.SetEnvironmentVariable("NL_FORK_PUBLIC_CONNECT_HOST", "play.example.com");
            var host = CreateHost();
            var masked = host.Relay.MaskEndpoint("rimworld://127.0.0.1:25555", "us-east", "sess123");
            Assert.Equal("rimworld://play.example.com:25555", masked.PublicConnectUrl);
            Assert.DoesNotContain("wss://", masked.PublicConnectUrl, StringComparison.OrdinalIgnoreCase);
        }
        finally
        {
            Environment.SetEnvironmentVariable("NL_FORK_PUBLIC_CONNECT_HOST", previous);
        }
    }
