...
When `NL_FLEET_ENABLED=true`, **non-native** manifest endpoints (`ws://`, `docker://`) are masked via relay template:

```
wss://relay-{region}.nl.example.com/fork/{session}
```

**T2-lite:** `rimworld://` and `minecraft://` stay native TCP. Loopback is rewritten to `NL_FORK_PUBLIC_CONNECT_HOST` ([NL_T2_LITE.md](NL_T2_LITE.md)). Do not expect RimWorld Together to speak WSS.
...
| GET | `/api/v1/t2-lite/status` | Public RimWorld connect host / ready |
