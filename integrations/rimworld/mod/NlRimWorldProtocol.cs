...
/// <summary>NL Integration Spec v1 decision returned to the RimWorld host (propose-then-commit).</summary>
public sealed record NlRimWorldDecision(bool Allowed, string? Message, string? Action)
{
    public static NlRimWorldDecision Allow() => new(true, null, null);

    public static NlRimWorldDecision Block(string? message, string? action = "kick") =>
        new(false, message, action);
}

/// <summary>Inbound action line from the NL session bus.</summary>
public sealed record NlRimWorldAction(
    string Action,
    string Player,
    string Event,
    string Decision,
...
/// <summary>Evaluates a colony event against NL (embedded fake or live WebSocket bus).</summary>
public interface INlRimWorldDecisionBus
{
    NlRimWorldDecision Evaluate(string eventName, string player, IReadOnlyDictionary<string, double> props);
}

/// <summary>Minimal JSON action-line parser (no extra packages — matches Paper NlWebSocketBridge).</summary>
public static class NlRimWorldActionParser
{
    public static NlRimWorldAction? TryParse(string line)
    {
    ...
    public static NlRimWorldDecision ToDecision(NlRimWorldAction? action)
    {
        if (action is null)
        {
        ...
        return block
            ? NlRimWorldDecision.Block(action.Message, action.Action)
            : NlRimWorldDecision.Allow();
    }
