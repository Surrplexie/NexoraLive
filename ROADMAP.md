...
**Built:** Phases 0–K + **Phase P** (fork runtime) + **Phase L** (platform identity) + **Phase M** (live social gate) + **Account verification** (email + TOTP 2FA) + **Unified NL login** (one account = identity + SP + streamer) + **Phase N** (fork catalog) + **Phase O** (fork orchestrator) + **Phase Q** (partnerships) + **Phase R** (NL Client) + **Phase S** (fleet ops + staging validation) + **Production dogfood track** + **VPS production deploy templates** + **Phase T0** (game adapter contract / template) + **Phase T1 RimWorld (full)** + **Phase T1 Kenshi (full)**.
**Next track:** T1 Cities: Skylines, or full T2 (TURN / unique game ports). T2-lite public RimWorld connect is in-repo ([docs/NL_T2_LITE.md](docs/NL_T2_LITE.md)).
...
- [x] **Account verification (email + 2FA)** — `AccountVerificationService`, `/account-verify.html`, `SpVerification` sync at admit — [docs/NL_ACCOUNT_VERIFICATION.md](docs/NL_ACCOUNT_VERIFICATION.md)
- [x] **Unified NL login & accounts** — register/login sessions, one id for identity + SP + streamer — [docs/NL_UNIFIED_LOGIN.md](docs/NL_UNIFIED_LOGIN.md)
...
- [x] **Twitch / YouTube / Kick OAuth** — browser OAuth at `/social-link.html` + per-player tokens (M.1 Twitch Helix, M.3 YouTube subscriptions, M.4 Kick PKCE linking; Kick follow API pending Kick platform)
...
- [x] **Live social dogfood track** — mock/live fixtures, E2E flow, operator UI — [docs/NL_LIVE_SOCIAL_DOGFOOD.md](docs/NL_LIVE_SOCIAL_DOGFOOD.md)
...
## Game expansion track — Phases T0–T8

Multi-phase plan to add **mod-backed**, **publisher**, and (research) **SP→MP** titles toward
the ideal NL path (NL hosts world, native client, app admit). Full detail:

**[docs/NL_GAME_EXPANSION_PLAN.md](docs/NL_GAME_EXPANSION_PLAN.md)** · **[docs/NL_GAME_ADAPTER.md](docs/NL_GAME_ADAPTER.md)**

| Phase | Summary | Status |
|-------|---------|--------|
| T0 | Integration contract + per-game adapter template | ✅ — `integrations/_template/`, `IGameForkAdapter`, `nl-game-adapter-validate.ps1` |
| T1 | RimWorld, Kenshi, Cities: Skylines (mod-backed MP) | 🟡 RimWorld ✅ — Kenshi / Skylines planned |
| T2 | Public connect / relay / VPS production | 🟡 **T2-lite ✅** — full T2 (TURN/game relay) planned |
| T3 | Desktop NL Client app (PC) | planned |
| T4 | Fragile mod titles (Fallout NV, Subnautica, …) | planned |
| T5 | Publisher Official tier pipeline | planned |
| T6 | Console identity + join | planned |
| T7 | NL Sync R&D (true SP shared worlds) | planned |
| T8 | Economy + community hub (extends Phase 6+) | planned |

### Phase T0 — Integration contract ✓

- [x] **`IGameForkAdapter`** — events, actions, connect scheme, health probe (`src/NL.Fork.Core/`)
- [x] **`adapter.manifest.json`** discovery + `GameForkAdapterRegistry` / checklist
- [x] **Template folder** — `integrations/_template/` (+ filled `example-game`)
- [x] **Reference manifests** — hello-fork, minecraft, minecraft-paper, beamng
- [x] **Scaffold** — `scripts/nl-game-adapter-scaffold.ps1`
- [x] **Validate** — `scripts/nl-game-adapter-validate.ps1` / `.sh` + CI job
- [x] **Docs** — [docs/NL_GAME_ADAPTER.md](docs/NL_GAME_ADAPTER.md)

### Phase T1 — RimWorld ✓ (full)

- [x] **Catalog** — `rimworld@1.0` AtOwnRisk, Steam **294100**
- [x] **`RimWorldForkRuntime`** — chat / combat / grief / draft / trade / rescue (propose-then-commit)
- [x] **Docker** — `nl-fork-rimworld:latest` sidecar + optional licensed Together dedicated (`/game`)
- [x] **ForkGameProfiles** — `rimworld://host:25555` TCP banner
- [x] **Harmony / Together bridge** — `integrations/rimworld/mod` (`NexoraLive.NLBridge`)
- [x] **`.nle`** — `samples/configs/rimworld.nle` (caps, grief, draft, trade)
- [x] **Identity** — mock ownership matrix includes 294100
- [x] **Dogfood** — `scripts/nl-dogfood-flow-rimworld.ps1` + social dogfood `-GameId rimworld` + production `-AllGames`
- [x] **Docs** — [docs/NL_RIMWORLD.md](docs/NL_RIMWORLD.md)

### Phase T1 — Kenshi ✓ (full)

- [x] **Catalog** — `kenshi@1.0` AtOwnRisk, Steam **233860**
- [x] **`KenshiForkRuntime`** — chat / combat / steal / raid / squad / trade / KO (propose-then-commit)
- [x] **Docker** — `nl-fork-kenshi:latest` sidecar + optional licensed install (`/game`)
- [x] **ForkGameProfiles** — `kenshi://host:23386` TCP banner
- [x] **Community-MP bridge** — `integrations/kenshi/mod` (`NexoraLive.NLKenshiBridge`)
- [x] **`.nle`** — `samples/configs/kenshi.nle` (caps, steal, raid, squad, trade)
- [x] **Identity** — mock ownership matrix includes 233860
- [x] **Dogfood** — `scripts/nl-dogfood-flow-kenshi.ps1` + social dogfood `-GameId kenshi` + production `-AllGames`
- [x] **Docs** — [docs/NL_KENSHI.md](docs/NL_KENSHI.md)
- [x] **Not** a VPS `NL_GA_REQUIRED_GAMES` launch blocker
- [ ] **Cities: Skylines** — third T1 title

### Phase T2-lite — public RimWorld connect ✓

- [x] **Public `forkConnectEndpoint`** — `rimworld://{NL_FORK_PUBLIC_CONNECT_HOST}:25555` (no loopback in VPS manifests)
- [x] **Native scheme kept** — fleet WSS mask does not replace RimWorld/Minecraft TCP
- [x] **NL Client clipboard** — `nativeConnectClipboard` + `/nl-client.html` hint
- [x] **VPS** — env + firewall **25555/tcp**; `GET /api/v1/t2-lite/status`
- [x] **Validate** — `scripts/nl-t2-lite-validate.ps1`
- [x] **Docs** — [docs/NL_T2_LITE.md](docs/NL_T2_LITE.md)
- [ ] **Full T2** — TURN for game ports, unique ports per session, cross-region dogfood

**Next game-expansion step:** T1 Cities: Skylines — or operator VPS signoff using T2-lite.
